...
class RequestToken {
	 ...
	 *                custom salt value to be used by `StringUtil::hash()`.
	 *              - `'type'` _string_: The hashing algorithm used by `StringUtil::hash()` when
	...
	public static function get(array $options = array()) {
		...
		if ($options['regenerate'] || !($token = $session::read($options['sessionKey']))) {
			$token = StringUtil::hash(uniqid(microtime(true)), $options);
