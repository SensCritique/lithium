...
class StringUtil {
	...
	/**
	 * Option flag used in `StringUtil::random()`.
	 */
	 ...
	 *
	 * {{{
	 * $bits = StringUtil::random(8); // 64 bits
	 ...
	 *              - `'encode'` _integer_: If specified, and set to `StringUtil::ENCODE_BASE_64`, the
	...
	/**
	 * Initializes `StringUtil::$_source` using the best available random number generator.
	 *
	 ...
	 *
	 * @see lithium\util\StringUtil::$_source
	 ...
	 *        - `'clean'`: A boolean or array with instructions for `StringUtil::clean()`.
	...
	/**
	 * Cleans up a `StringUtil::insert()` formatted string with given `$options` depending
	 * on the `'clean'` option. The goal of this function is to replace all whitespace
	 * and unneeded mark-up around place-holders that did not get replaced by `StringUtil::insert()`.
	 *
